...
function find_all_departament() {
  global $db;
  $results = array();
  $sql = "SELECT DISTINCT departamento FROM headcount";
  $result = find_by_sql($sql);
  return $result;
}
function find_all_participantes($id) {
  global $db;
  $results = array();
  $sql = "SELECT * FROM participantes_form WHERE id_formulario = '{$id}'";
  $result = find_by_sql($sql);
  return $result;
}
/**
...
function find_all_by_table($table) {
  global $db;
...
function find_all_by_category($table, $category) {
  global $db;
  $sql  = "SELECT * FROM {$table} WHERE Categoria = '{$category}' ORDER BY Estatus ASC";

  return find_by_sql($sql);
}

function find_all_by_category1($table, $category) {
  global $db;
  
  if ($category == 'Camaras') {
    // Realizar una consulta que una los datos de la tabla de Cámaras y NVR.
    // Asumo que la tabla se llama 'tu_tabla' y el campo 'idnvr' existe en ambas tablas.
    // Ajusta el nombre de la tabla y los campos según sea necesario.
    $sql = "SELECT c.*, n.id AS id_nvr, n.Categoria AS categoria_nvr, n.descricion AS otro_campo_nvr
            FROM {$table} c
            LEFT JOIN {$table} n ON c.idnvr = n.idnvr
            WHERE c.Categoria = 'Camaras' AND n.Categoria = 'NVR'
            ORDER BY c.Estatus ASC, c.ID ASC";
  } else {
    // Realizar la consulta habitual si no es la categoría 'Cámaras'.
    $sql = "SELECT * FROM {$table} WHERE Categoria = '{$category}' ORDER BY Estatus ASC,ID ASC";
  }

  return find_by_sql($sql);
}

function find_all_by_nvr($categoria, $id) {
  global $db;
  $sql  = "SELECT * FROM servidores WHERE idnvr = '{$id}' AND Categoria = '{$categoria}' ORDER BY Estatus ASC";
